...
def DBN_import_dataset(image_size):
    ...
    train_image = train_image.reshape(-1, image_width_in_pixels*image_height_in_pixels*color_channel) / 255.0
    val_image = val_image.reshape(-1, image_width_in_pixels*image_height_in_pixels*color_channel) / 255.0
...
def DBN_create_model(batch_size):
    ...
    logistic = LogisticRegression(solver='sag', max_iter=10000, multi_class='multinomial')
...
DBN_save_model(pipeline, 'dbn_2RBM_256.joblib')