...
class CNNModel(nn.Module):
    ...
    #def _get_classifier_input_dim(self, input_shape):
    #    return self.features(torch.zeros(1, *input_shape)).shape[1]
...
model.load_state_dict(torch.load('pytorch_cnn_origin.pth', map_location=device))
model.to(device)
...
def foo():
    # Open the csv file
    with open('pytorch_cnn_or_120_pred1.csv', 'w', newline='') as file:
